...
/**
 * Shared body for the state-association landing pages (/mrla, /ira, /frla,
 * more to come). Every one of these came from the same agency mockup with
 * only the association's identity substituted — this component holds the
 * copy that's byte-identical across all of them; each page passes in only
 * what actually differs (name, badge, Calendly link, membership wording).
 *
 * Wording rule (applies to every association here, not just one): these
 * associations reserve "partner" for their own endorsed partners — never
 * use that word in copy, headings, alt text, URL slugs, class names, or
 * (per the IRA/FRLA mockups specifically) UTM values either. Use "member".
 * MRLA's own Calendly UTM predates that stricter rule and still carries
 * utm_medium=partner — that's a UTM param, not visible copy, so it was left
 * as supplied rather than diverging from what marketing configured in
 * Calendly.
 */

export type AssociationLandingPageProps = {
  /** Short public name, e.g. "MRLA" — used in "For {abbr} members", the
   *  offer heading, and the CTA notes. */
  abbr: string;
  calendlyUrl: string;
  /** Full association name, shown next to the hero badge pill. */
  fullName: string;
  badgeSrc: string;
  badgeWidth: number;
  badgeHeight: number;
  /** Rendered height in px — mockups vary (76 for MRLA's tall badge, 62 for
   *  IRA/FRLA's wide ones). */
  badgeDisplayHeight: number;
  badgeAlt: string;
  /** The member-bar paragraph, verbatim from the mockup — phrasing ("a
   *  Proud Member of" vs "a member of") isn't uniform enough to template. */
  memberBarText: string;
};
...
// Subset of the brands referenced in the source mockups that we already
// have logo assets for in this repo (same set TrustedBy.tsx draws from). A
// few names the mockups list (Happea's, Think Hospitality, El Club de la
        ...
        <div className="mx-auto max-w-[1080px] px-6 md:px-10">
          ...
          <div className="mt-8 flex flex-wrap items-center gap-5">
            ...
            <span className="text-sm text-[#251f21]/50">
              {abbr} members get 50% off the setup fee.
            </span>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto flex max-w-[1080px] flex-wrap items-center gap-6 px-6 md:px-10">
          <div className="flex shrink-0 items-center rounded-2xl border border-[#251f21]/10 bg-white px-3.5 py-2.5">
            <Image
              ...
              unoptimized
              className="w-auto"
              style={{ height: badgeDisplayHeight }}
            />
          </div>
